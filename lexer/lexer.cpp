...
namespace Legion
{
	...
	Lexer::Lexer(StringPool &string_pool, MemoryPool &memory_pool, Document &document) : string_pool(string_pool), document(document), memory_pool(memory_pool), lexeme(*this), keywords(string_pool)
	{
		setup_jump_table();
	}
